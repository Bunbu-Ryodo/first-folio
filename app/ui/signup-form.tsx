    ...
    return (
            <div className="flex flex-col md-w:1/3">
                ...
                <form action={dispatch} className="w-full">
                    <label className="text-monokaiPink text-informational">&lt;Email&gt;</label>
                    ...
                    {state?.errors && state.errors?.email && state.errors.email.map((error: string) => (
                        <div key={error} className="mt-2 mb-2 text-indianRed">
                        {error}
                        </div>
                    ))}
                    <label className="text-monokaiPink text-informational">&lt;Create a Password&gt;</label>
                    ...
                    <label className="text-monokaiPink text-informational">&lt;Confirm Password&gt;</label>
                    ...
                    {state?.errors && state.errors?.confirmPassword && state.errors.confirmPassword.map((error: string) => (
                        <div key={error} className="mt-2 mb-2 text-informational text-indianRed">
                            {error}
                        </div>
                    ))}
                    ...
                    {state?.message && state.message &&
                        <div key={state.message} className="mt-2 mb-2 text-informational text-indianRed">
                            {state.message}
                        </div>
                    }
                </form>
